...
(function(){
  ...
  doLogin=async function(){
    ...
    try{
      ...
      await loadBootstrap();
      err.classList.remove('show');hide('pgLogin');if(role('super_admin')){initDash();openServices('users');}else showMonthPicker();
    }catch(e){err.textContent=humanError(e.message);err.classList.add('show');}
    finally{A.loginBusy=false;busy(btn,false);}
  };

  async function restoreSession(){
    try{var params=new URLSearchParams(location.search),resetToken=params.get('reset_token');if(resetToken){buildResetFormUI(resetToken);pg('pgLogin');return;}var st=await api('/api/app/auth/status');A.setupRequired=!!st.setup_required;if(A.setupRequired||params.get('setup')==='1'){buildSetupUI();pg('pgLogin');return;}var meErr=null;for(var attempt=0;attempt<8;attempt++){try{await api('/api/app/auth/me?ts='+Date.now());meErr=null;break;}catch(e){meErr=e;await new Promise(function(r){setTimeout(r,500+attempt*500);});}}if(meErr){try{var authSession=await api('/api/auth/session?ts='+Date.now());if(!authSession||!authSession.user)throw meErr;}catch(_e){throw meErr;}}await loadBootstrap();hide('pgLogin');if(role('super_admin')){initDash();openServices('users');}else showMonthPicker();}
    catch(e){if(A.setupRequired)buildSetupUI();else buildLoginUI();pg('pgLogin');setTimeout(function(){var x=$('lgEmail');if(x)x.focus();},60);}
  }
  ...
  async function loadBootstrap(){
    var j=await api('/api/app/bootstrap');
  ...
  mpConfirm=async function(){
    ...
    try{
      ...
      A.cycle=cycle;svMo({m:String(_mpSel),y:String(y)});await loadCycleData();hide('pgMonth');initDash();
  ...
  async function loadCycleData(){
    ...
    var jobs=[api('/api/app/bootstrap')];
    ...
    var res=await Promise.all(jobs);A.data=res[0];A.permissions=A.data.permissions||[];A.roles=A.data.roles||[];A.cycle=findCycle(A.cycle.month,A.cycle.year)||A.cycle;
    A.cycleData={exclusions:res[1].exclusions||[],attendance:res[2].attendance||[],attendanceEntries:res[2].entries||[]};
    hydrateState();
  }
  ...
  function hydrateState(){
    ...
    var mo=selectedMonth();if(mo&&$('moChip')){$('moChip').textContent=MONTHS[mo.m-1]+' '+mo.y;$('moChip').style.display='inline-block';}
  }
  ...
  initDash=function(){
    ...
    installToolbarButtons();renderGrid();dAlertShow();
  };
  ...
  renderGrid=function(){
    ...
    function addBtn(){if(!can('employees.create')&&!can('employees.update'))return null;var a=document.createElement('button');a.className='emp-add-card';a.type='button';a.onclick=openEmpMo;a.innerHTML='<span class="eac-plus"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg></span><span class="eac-t">إدارة الموظفين</span>';return a;}
  ...
  _eRender=function(){_origERender();if(!can('employees.update'))document.querySelectorAll('#empMo .em-ic[id^="neb_"],#empMo .em-ic.ok').forEach(function(x){x.style.display='none';});if(!can('criteria.manage'))document.querySelectorAll('#empMo .em-acts .em-ic:nth-child(3)').forEach(function(x){x.style.display='none';});if(!can('employees.delete'))document.querySelectorAll('#empMo .em-ic.del').forEach(function(x){x.style.display='none';});};
  ...
  function ensureServiceModal(){if($('svcMo'))return;var wrap=document.createElement('div');wrap.innerHTML='<div id="svcMo" class="em-ov" onclick="if(event.target===this)closeServices()"><div class="em-box" style="max-width:900px;max-height:90vh;overflow:hidden"><div class="em-head"><div><div class="em-title">الخدمات والإدارة</div><div class="em-sub" id="svcSub">مرتبطة مباشرة بقاعدة بيانات Neon</div></div><button class="em-x" onclick="closeServices()"><svg viewBox="0 0 24 24"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button></div><div id="svcTabs" style="display:flex;gap:7px;flex-wrap:wrap;padding:0 26px 14px"></div><div id="svcBody" class="em-list" style="padding-bottom:18px"></div><div class="em-foot"><span class="em-auto" id="svcFoot">الصلاحيات مطبقة من قاعدة البيانات</span><button class="em-save" style="flex:0 0 auto;padding:0 30px" onclick="closeServices()">تم</button></div></div></div>';document.body.appendChild(wrap.firstChild);var body=$('svcBody');if(body){body.addEventListener('click',function(e){var t=e.target.closest('.role-perm-toggle');if(t){e.preventDefault();var id=t.getAttribute('data-role-id');if(id)window.toggleRolePerm(id);}});body.addEventListener('wheel',function(e){if(body.scrollHeight>body.clientHeight){body.scrollTop+=e.deltaY;e.preventDefault();e.stopPropagation();}},{passive:false});}var modal=$('svcMo');if(modal)modal.addEventListener('wheel',function(e){if(e.target===modal){e.preventDefault();e.stopPropagation();}},{passive:false});}
  function tabs(){var t=[];if(can('evaluations.view'))t.push(['cycles','الدورات']);if(can('evaluations.review')||can('evaluations.approve')||can('evaluations.publish')||can('evaluations.reopen'))t.push(['review','المراجعة']);if(can('users.view'))t.push(['users','المستخدمون']);if(can('branches.manage')||can('departments.manage')||can('job_titles.manage')||can('employees.update'))t.push(['org','الهيكل']);if(can('roles.manage'))t.push(['roles','الصلاحيات']);if(can('settings.manage'))t.push(['settings','الإعدادات']);if(can('reports.view'))t.push(['reports','التقارير']);if(can('audit_logs.view'))t.push(['audit','سجل العمليات']);t.push(['notifications','الإشعارات']);if(cu&&cu.employee_id)t.push(['portal','ملفي']);return t;}
  window.openServices=async function(tab){ensureServiceModal();$('svcMo').classList.add('show');var ts=tabs(),active=tab||ts[0][0];$('svcTabs').innerHTML=ts.map(function(x){return '<button class="btn '+(x[0]===active?'':'btn-g')+'" style="'+(x[0]===active?'background:var(--blue);color:#fff':'')+'" onclick="svcTab(\''+x[0]+'\')">'+x[1]+'</button>';}).join('');await renderService(active);};
  window.closeServices=function(){$('svcMo').classList.remove('show');};
  ...
  window.toggleRolePerm=function(id){var b=$('rolePerm_'+id),a=$('roleArrow_'+id);if(!b)return;var open=b.style.display==='none';b.style.display=open?'block':'none';if(a)a.textContent=open?'⌃':'⌄';};
