...
(function(){
  ...
  function humanError(code){
    var m={
      ...
      ORGANIZATION_NOT_FOUND:'لم يتم العثور على منشأة السويد',EMPLOYEE_ALREADY_HAS_USER:'هذا الموظف لديه حساب مستخدم مسبقاً',INVALID_EMPLOYEE:'الموظف المختار غير موجود',INVALID_INPUT:'تحقق من البيانات المدخلة'
    };
  ...
  async function svcUsers(b){var j=await api('/api/app/users');A.users=j;var admin=await loadAdmin(),empRes=await api('/api/app/employees?limit=300');var linked={};(j.users||[]).forEach(function(u){if(u.employee_id)linked[String(u.employee_id)]=u;});var employeeChoices=(empRes.employees||[]).filter(function(e){return ['terminated','resigned'].indexOf(e.status)<0;});var roleOpts=(j.roles||[]).map(function(r){return '<option value="'+r.id+'" data-code="'+h(r.code)+'">'+h(r.name_ar)+'</option>';}).join(''),empOpts='<option value="">اختر الموظف</option>'+employeeChoices.map(function(e){var u=linked[String(e.id)],disabled=u?' disabled':'',tag=u?' — لديه حساب':'';return '<option value="'+h(e.id)+'" data-name="'+h(e.full_name||'')+'" data-email="'+h(e.email||'')+'"'+disabled+'>'+h((e.full_name||'موظف')+tag)+'</option>';}).join('');var form='';if(can('users.create')&&can('roles.manage'))form='<div class="em-add" style="align-items:flex-end"><div class="em-inp-wrap"><select class="em-inp" style="padding:0 14px" id="uEmp" onchange="svcUserEmployeeChange()">'+empOpts+'</select></div><div class="em-inp-wrap"><input class="em-inp" style="padding:0 14px" id="uName" placeholder="الاسم" readonly></div><div class="em-inp-wrap"><input class="em-inp" style="padding:0 14px;direction:ltr" id="uEmail" placeholder="email@company.com"></div><div class="em-inp-wrap"><input class="em-inp" style="padding:0 14px;direction:ltr" id="uPass" type="password" placeholder="كلمة المرور"></div><div class="em-inp-wrap"><select class="em-inp" style="padding:0 14px" id="uRole" onchange="svcScopeChange()">'+roleOpts+'</select></div><div class="em-inp-wrap"><select class="em-inp" style="padding:0 14px" id="uScopeType" onchange="svcScopeChange()"><option value="organization">المنشأة كاملة</option><option value="branch">فرع محدد</option><option value="department">إدارة محددة</option><option value="assigned_employees">الموظفون المسندون فقط</option></select></div><div class="em-inp-wrap" id="uScopeIdWrap" style="display:none"><select class="em-inp" style="padding:0 14px" id="uScopeId"></select></div><button class="em-addbtn" onclick="svcCreateUser()">إضافة مستخدم</button></div>';var list=(j.users||[]).map(function(u){var act=can('users.update')?actionBtn(u.is_active?'تعطيل':'تفعيل','svcToggleUser('+idjs(u.id)+','+(!u.is_active)+')'):'';return row(u.full_name||u.email,(u.email||'')+(u.employee_id?' · مرتبط بموظف':'')+(u.is_active?' · نشط':' · معطل'),act);}).join('');b.innerHTML=form+list+(list?'':'<div class="em-empty">لا توجد حسابات</div>');if(form){svcUserEmployeeChange();svcScopeChange();}}
  window.svcUserEmployeeChange=function(){var sel=$('uEmp'),name=$('uName'),email=$('uEmail');if(!sel||!name||!email)return;var opt=sel.options[sel.selectedIndex];name.value=opt?String(opt.getAttribute('data-name')||''):'';var empEmail=opt?String(opt.getAttribute('data-email')||''):'';email.value=empEmail;email.readOnly=!!empEmail;};
