...
export async function uploadDesignAsset(
  ...
  const key = `organizations/${organizationId}/designs/${folder}/${randomUUID()}.${extensions[file.type]}`;
  ...
  return {
    key,
    url: `/api/design-assets/${key.split('/').map(encodeURIComponent).join('/')}`,
  };
}
...
export async function deleteDesignAsset(key: string) {
  if (!key) return;
  await client().send(new DeleteObjectCommand({ Bucket: DESIGN_BUCKET, Key: key }));
}
