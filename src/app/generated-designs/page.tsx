...
export default function GeneratedPage() {
  ...
  useEffect(() => {
    setLoading(true);
    designApi(`/api/generated-designs?page=${page}&limit=12`)
      .then((r) => {
        setItems(r.designs);
        setTotal(r.total);
      })
    ...
    <DesignShell>
      <main className="ds-main">
        <div className="ds-head">
          <div>
            <h1>سجل التصاميم</h1>
            <p>التصاميم الصادرة والمحفوظة داخل النظام.</p>
          </div>
        </div>
        {loading ? (
            ...
            {items.map((x) => (
              <article className="dst-card" key={x.id} style={{ padding: 0, overflow: 'hidden' }}>
                ...
                <div className="ds-card-body">
                  <h3>{x.template_name}</h3>
                  <div className="ds-meta">
                    <span>المستخدم: {x.generated_by_name || "—"}</span>
                    <span>الموظف: {x.employee_name || "—"}</span>
                    <span>الصيغة: {String(x.image_format).toUpperCase()}</span>
                    <span>
                      {x.width}×{x.height}
                    </span>
                    <span>
                  ...
                  {x.image_url && (
                    <div className="ds-actions">
                      <a className="primary" href={x.image_url} download>
                        تحميل
                      </a>
                    </div>
                  )}
