...
export async function GET(req:NextRequest){
  try{
    const c=await requireUser(),u=new URL(req.url);const limit=Number(u.searchParams.get('limit')||100),offset=Number(u.searchParams.get('offset')||0),search=u.searchParams.get('search')||'';
    ...
    const [titles,cycles,criteria,ratings,penalties,settings,assignments,evals,targets,attendance]=await Promise.all([
      ...
      ids.length?pool.query(`select id,cycle_id,employee_id,evaluation_type,template_id,due_at from public.evaluation_assignments where evaluator_user_id=$1::uuid and employee_id=any($2::uuid[])`,[c.user.id,ids]):Promise.resolve({rows:[]} as any),
      ids.length?pool.query(`select id,assignment_id,cycle_id,employee_id,evaluation_type,status,weighted_score,final_score,result_label,notes,submitted_at,published_at,template_snapshot from public.evaluations where evaluator_user_id=$1::uuid and employee_id=any($2::uuid[])`,[c.user.id,ids]):Promise.resolve({rows:[]} as any),
      ids.length?pool.query(`select id,cycle_id,employee_id,target_amount,achieved_amount,last_edit_reason,source from public.sales_targets where employee_id=any($1::uuid[])`,[ids]):Promise.resolve({rows:[]} as any),
      ids.length?pool.query(`select id,cycle_id,employee_id,base_score,final_score,notes,status,updated_at from public.attendance_evaluations where employee_id=any($1::uuid[])`,[ids]):Promise.resolve({rows:[]} as any)
    ]);
