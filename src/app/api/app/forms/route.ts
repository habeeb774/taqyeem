...
async function nextDocumentNumber(
  ...
  const prefix = z
    ...
    .min(1)
    .max(30)
    .parse(body.prefix || "HR");
  const key = z
    .string()
    .min(1)
    .max(80)
    .parse(body.sequence_key || prefix);
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (query.get("approvals") === "1") {
      const canReviewAll = context.roles.some(
        (role) => role.code === "super_admin" || role.code === "hr_admin",
      );
      const result = await pool.query(
    ...
    const visibleEmployeeIds = viewAll
      ? []
      : await allVisibleEmployeeIds(context);
    const result = await pool.query(
      `
      ...
      order by d.updated_at desc limit $9 offset $10`,
      [
        context.organizationId,
        type,
        status,
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const template = await saveFormTemplate(
      context,
      templateSchema.parse(await req.json()),
    );
  ...
  try {
    ...
    if (transitionAction.success && transitionActions.has(transitionAction.data)) {
      ...
      return jsonOk({
        ...
        current_approval_step: result.currentApprovalStep,
      });
    }

    must(context, "forms.create");
    const value = documentSchema.parse(body);
    const employeeSnapshot = value.employee_id
      ? await getEmployeeFormProfile(context, value.employee_id)
      : value.employee_snapshot || {};
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const documentNumber = z
      ...
      .min(1)
      .max(120)
      .parse(new URL(req.url).searchParams.get("document_no") || "");
    const client = await pool.connect();
    try {
      await client.query("begin");
      const result = await client.query(
        `select id,employee_id,status,form_type,created_by from public.form_documents where organization_id=$1::uuid and document_no=$2 and deleted_at is null for update`,
        [context.organizationId, documentNumber],
      );
      const document = result.rows[0];
      if (!document)
        throw Object.assign(new Error("NOT_FOUND"), { status: 404 });
      const ownDraft =
        document.form_type === "__draft__" &&
        String(document.created_by) === String(context.user.id) &&
        can(context, "forms.create");
      if (!ownDraft) must(context, "forms.delete");
      if (document.employee_id && !can(context, "forms.view_all"))
        await assertEmployeeAccess(context, String(document.employee_id));
      await client.query(
        `update public.form_documents set deleted_at=now(),updated_by=$1::uuid,updated_at=now() where id=$2::uuid`,
        [context.user.id, document.id],
      );
      await client.query(
        `update public.form_submissions set deleted_at=now(),updated_at=now() where organization_id=$1::uuid and document_number=$2`,
        [context.organizationId, documentNumber],
      );
      await client.query(
        `insert into public.audit_logs(organization_id,user_id,action,entity_type,entity_id,old_values,new_values) values($1::uuid,$2::uuid,'form.delete','form_document',$3::uuid,$4::jsonb,$5::jsonb)`,
        [
          context.organizationId,
          context.user.id,
          document.id,
