...
import { z } from 'zod';
import { requireUser, jsonError, must, assertEmployeeAccess, can } from '@/server/context';
import { pool } from '@/db';
import { allVisibleEmployeeIds } from '@/db/queries/security';
import { resolveTemplate, templateItems, recomputeEvaluation } from '@/db/queries/evaluations';
...
async function loadAnswerMeta(queryable: { query: Function }, templateId: string, criterionId: string) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (evaluationId) {
      ...
      if (!ev.rows[0]) throw Object.assign(new Error('NOT_FOUND'), { status: 404 });
      ...
      x.evaluation_cycles = { name: x.cycle_name, month: x.month, year: x.year };
      return NextResponse.json({ ok: true, evaluation: x, answers: ans.rows });
    }

    if (queue === 'review') {
      if (!can(c, 'evaluations.review') && !can(c, 'evaluations.approve') && !can(c, 'evaluations.publish')) {
        throw Object.assign(new Error('FORBIDDEN'), { status: 403 });
      }
      ...
      const r = await pool.query(
        ...
        [ids, ['submitted', 'reviewed', 'approved', 'published', 'locked'], cycleId || null],
      );
      return NextResponse.json({
        ...
        evaluations: r.rows.map((x: any) => ({
          ...
          evaluation_cycles: { name: x.cycle_name, month: x.month, year: x.year },
        })),
      });
    }
    ...
    if (employeeId) {
      ...
      if (!visibleIds.length) return NextResponse.json({ ok: true, assignments: [] });
  ...
  try {
    ...
    if (action === 'ensure') {
      ...
      await withNeonTransaction(async (tx) => {
        ...
        let e = await tx.query(`select id from public.evaluations where assignment_id=$1::uuid limit 1`, [assignmentId]);
        if (!e.rows[0]) {
          e = await tx.query(
            ...
            [assignmentId, cycleId, employeeId, c.user.id, templateId, JSON.stringify({ template_id: templateId, criteria: items })],
          );
        }
    ...
    if (action === 'autosave') {
      must(c, 'evaluations.edit');
      const p = z.object({ evaluation_id: z.string().uuid(), answer: answerSchema.optional(), notes: z.string().optional() }).parse(b);
      const ev = await pool.query(
      ...
      if (String(e.evaluator_user_id) !== c.user.id || e.status !== 'draft') {
        throw Object.assign(new Error('evaluation_not_editable'), { status: 409 });
      }
      if (p.answer) {
        const m = await loadAnswerMeta(pool, e.template_id, p.answer.criterion_id);
        ...
        await pool.query(
          ...
          [p.evaluation_id, p.answer.criterion_id, m.name, m.description, Number(m.weight), Number(m.max_score), p.answer.score, p.answer.comment || ''],
        );
      }
      if (p.notes !== undefined) {
        await pool.query(`update public.evaluations set notes=$2,updated_at=now() where id=$1::uuid`, [p.evaluation_id, p.notes]);
      }
    ...
    if (action === 'save') {
      must(c, 'evaluations.edit');
      const p = z.object({ evaluation_id: z.string().uuid(), notes: z.string().default(''), answers: z.array(answerSchema) }).parse(b);
      const ev = await pool.query(
        ...
        for (const a of p.answers) {
          ...
          await tx.query(
            `insert into public.evaluation_answers(evaluation_id,criterion_id,criterion_name_snapshot,criterion_description_snapshot,weight_snapshot,max_score_snapshot,score,comment)
             values($1::uuid,$2::uuid,$3,$4,$5,$6,$7,$8)
             on conflict(evaluation_id,criterion_id) do update set score=excluded.score,comment=excluded.comment,updated_at=now()`,
            [p.evaluation_id, a.criterion_id, m.name, m.description, Number(m.weight), Number(m.max_score), a.score, a.comment || ''],
          );
        }
        await tx.query(`update public.evaluations set notes=$2,updated_at=now() where id=$1::uuid`, [p.evaluation_id, p.notes]);
      });
      return NextResponse.json({ ok: true, score: await recomputeEvaluation(p.evaluation_id) });
    }

    if (['submitted', 'reviewed', 'approved', 'published', 'locked', 'draft'].includes(action)) {
      const id = z.string().uuid().parse(b.evaluation_id);
      const ev = await pool.query(`select * from public.evaluations where id=$1::uuid`, [id]);
      const e: any = ev.rows[0];
      if (!e) throw new Error('NOT_FOUND');
      await assertEmployeeAccess(c, String(e.employee_id));
      const perm: any = {
      ...
      if (!allowed[action].includes(e.status)) throw Object.assign(new Error('invalid_transition'), { status: 409 });
      if (action === 'submitted') {
        ...
        if (Number(zc.answered) < Number(zc.required)) throw new Error('evaluation_incomplete');
        if (Number(zc.missing_comment) > 0) throw new Error('required_comment_missing');
      }
      const col: any = {
      ...
      if (action === 'draft' && reasonText.length < 3) throw new Error('REOPEN_REASON_REQUIRED');
      await withNeonTransaction(async (tx) => {
        await tx.query(
        ...
        await tx.query(
          ...
          [c.organizationId, c.user.id, `evaluation.${action}`, id, JSON.stringify({ status: e.status }), JSON.stringify({ status: action }), reason],
        );
      });
