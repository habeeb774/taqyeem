  ...
  try {
    ...
    return NextResponse.json({
      ...
      users: users.rows.map((user: any) => ({
        ...user,
        roles: userRoles.rows.filter((role: any) => String(role.user_id) === String(user.id)),
        scopes: userScopes.rows.filter((scope: any) => String(scope.user_id) === String(user.id)),
      })),
      roles: roles.rows,
...
export async function POST(request: NextRequest) {
  try {
    ...
    await withNeonTransaction(async (transaction) => {
      ...
      const userRole = await transaction.query(
